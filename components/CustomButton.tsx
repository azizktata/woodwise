...
export default function CustomButton({ label, className, inverted, asChild, href }: { label: string, className?: string, inverted?: boolean, asChild?: boolean, href: string }) {
  return (
    <Button asChild={asChild} className={cn("bg-gradient text-white font-bold px-7 py-6 md:px-11 md:py-7 text-sm hover:opacity-80 rounded-xl", { "bg-white hover:bg-gray-100 border border-woodPrimary rounded-full": inverted }, className)}>
